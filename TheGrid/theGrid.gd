...
func generate_hex_grid():
	...
	var hex_width = hex_size.x
	var hex_height = hex_size.y

	# Calculate grid dimensions
	var grid_width = (cols - 1) * hex_size.x * 0.75 + hex_size.x
	var grid_height = (rows - 1) * hex_size.y * 0.65 + hex_size.y
	var canvas_center = get_viewport_rect().size / 2
	var grid_offset = canvas_center - Vector2(grid_width / 2, grid_height / 2)

	for row in range(rows):
		for col in range(cols):
			var x_pos = col * hex_size.x * 0.75 + row * hex_size.x * 0.4 + grid_offset.x
